...
labels = [f"$2^{{{int(np.log2(bs))}}}$" for bs in block_sizes]

normal_df = pd.read_csv("ResultN/access_bsize_20.csv").iloc[:5]
anamorphic_df = pd.read_csv("ResultN/aaccess_bsize_20.csv").iloc[:5]
...
plt.savefig("ResultN/Access_StackedBar_N20_vs_BlockSize.pdf", format='pdf', bbox_inches='tight', dpi=300)
plt.show()
