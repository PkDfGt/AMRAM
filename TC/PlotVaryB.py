...
normal_df = pd.read_csv("Result/compact_normal_B_N20_vary.csv")
normal_df["time_s"] = normal_df["time_ms"] / 1000
anam_df = pd.read_csv("Result/compact_anamorphic_B_N20_vary.csv")
...
yticks = [2**i for i in range(2, 11, 2)]  # 2^0,2^2,2^4,2^6,2^8,2^10
ax.set_yticks(yticks)
ax.set_yticklabels([f"$2^{{{i}}}$" for i in range(2, 11, 2)])
ax.set_ylim((1 << 2) * 0.9, (1 << 10) * 1.1)

norm = mcolors.Normalize(vmin=0, vmax=len(threads)-1)

plt.savefig("Result/Com_VaryB_N20_Bar.pdf", format='pdf', bbox_inches='tight', dpi=300)
plt.show()
