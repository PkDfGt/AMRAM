...
normal_df = pd.read_csv("Result/compact_normal_N_vary.csv")
normal_df["time_s"] = normal_df["time_ms"] / 1000
anam_df = pd.read_csv("Result/compact_anamorphic_N_vary.csv")
anam_df["time_s"] = anam_df["time_ms"] / 1000

Ns = sorted(normal_df["N"].unique())
threads = sorted(normal_df["threads"].unique())

normal_cmap = LinearSegmentedColormap.from_list(
    "light_blues", cm.Greens(np.linspace(0.2, 0.6, 256))
)
anam_cmap = LinearSegmentedColormap.from_list(
    "light_greens", cm.Blues(np.linspace(0.2, 0.6, 256))
)
...
def plot_stacked_bars_with_diff_cmap(ax, df, x_positions, offset, cmap, threads):
    ...
    for xi, N in enumerate(Ns):
        sub = df[df["N"] == N][["threads", "time_s"]].copy()
...
plt.savefig("Result/Com_VaryN_Bar.pdf", format='pdf', bbox_inches='tight', dpi=300)
plt.show()
